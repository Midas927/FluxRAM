...
2. The app embeds only the public key for verification.
3. The private key must be stored outside the public repository and must not be uploaded to GitHub or sent to customers.
4. Pro keys are generated by an internal licensing tool outside the public source distribution.
...
1. `fluxram-license.private-key.xml` must be stored only in an internal secure location.
2. The internal Pro key generator, private key and licensing scripts must not enter the public GitHub repository.
3. Public download packages should be distributed through GitHub Release assets, not committed to the source tree.
4. Before production release, replace the embedded public key and code-sign the release executable.
...
9. GitHub repository does not contain `dist`, `bin`, `obj`, `.secrets`, private keys or logs.
